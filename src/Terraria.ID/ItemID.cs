...
namespace Terraria.ID
{
	public class ItemID
	{
		public class Sets
		{
			...
			public static bool[] gunProj = Factory.CreateBoolSet(3475, 3540);

			public static bool[] GamepadWholeScreenUseRange = Factory.CreateBoolSet(1326, 1256, 1244, 3014, 113, 218, 495, 114, 496, 2796, 494, 3006, 65, 1931, 3570, 2750, 3065, 3029, 3030, 1309, 2364, 2365, 2551, 2535, 2584, 1157, 2749, 1802, 2621, 3249, 3531, 3474, 2366, 1572, 3569, 3571, 1299, 1254);
			...
			public static float?[] LockOnAimCompensation = Factory.CreateCustomSet<float?>(null, new object[8]
			{
				(short)1336,
				0.2f,
				(short)157,
		...
		private static void FillItemNameCacheArray<IdClass, IdType>(string[] nameCache) where IdType : IConvertible
		{
			for (int i = 0; i < nameCache.Length; i++)
			{
				nameCache[i] = string.Empty;
			}
				...
				select f).ToList().ForEach(delegate(FieldInfo field)
			{
				...
				{
					nameCache[num] = Regex.Replace(field.Name, pattern, "$1 $2");
				}
			});
		}

		public static void GenerateItemNameList(string[] list)
		{
			FillItemNameCacheArray<ItemID, short>(list);
		}
	}
}
