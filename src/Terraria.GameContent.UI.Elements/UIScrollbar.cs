...
namespace Terraria.GameContent.UI.Elements
{
	internal class UIScrollbar : UIElement
	{
		...
		private Rectangle GetHandleRectangle()
		{
			...
			return new Rectangle((int)ınnerDimensions.X, (int)(ınnerDimensions.Y + ınnerDimensions.Height * (_viewPosition / _maxViewSize)) - 3, 20, (int)(ınnerDimensions.Height * (_viewSize / _maxViewSize)) + 7);
		}
		...
		private void DrawBar(SpriteBatch spriteBatch, Texture2D texture, Rectangle dimensions, Color color)
		{
			...
			spriteBatch.Draw(texture, new Rectangle(dimensions.X, dimensions.Y, dimensions.Width, dimensions.Height), new Rectangle(0, 9, texture.Width, 2), color);
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			if (_isDragging)
			{
				float num = UserInterface.ActiveInstance.MousePosition.Y - ınnerDimensions.Y - _dragYOffset;
				_viewPosition = MathHelper.Clamp(num / ınnerDimensions.Height * _maxViewSize, 0f, _maxViewSize - _viewSize);
			}
		...
		public override void MouseDown(UIMouseEvent evt)
		{
			...
			if (evt.Target == this)
			{
				...
				{
					CalculatedStyle ınnerDimensions = GetInnerDimensions();
					float num = UserInterface.ActiveInstance.MousePosition.Y - ınnerDimensions.Y - (float)(handleRectangle.Height >> 1);
					_viewPosition = MathHelper.Clamp(num / ınnerDimensions.Height * _maxViewSize, 0f, _maxViewSize - _viewSize);
				}
			}
		}
