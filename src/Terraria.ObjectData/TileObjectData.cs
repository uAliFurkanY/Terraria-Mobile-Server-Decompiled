...
namespace Terraria.ObjectData
{
	public class TileObjectData
	{
		...
		private void SetupBaseObject()
		{
			...
			SubTiles = new List<TileObjectData>(419);
		...
		public static void Initialize()
		{
			...
			_data = new List<TileObjectData>(419);
			for (int i = 0; i < 419; i++)
			{
				_data.Add(null);
			}
			...
			newTile.HookPlaceOverride = new PlacementHook(WorldGen.PlaceXmasTree_Direct, -1, 0, true);
			...
			int[] array2 = newSubTile.AnchorValidWalls = new int[1];
			addSubTile(3);
			newSubTile.CopyFrom(StyleDye);
			int[] array4 = newSubTile.AnchorValidWalls = new int[1];
			addSubTile(4);
			newSubTile.CopyFrom(StyleDye);
			...
			{
				newAlternate.CopyFrom(newTile);
				newAlternate.Origin = new Point16(0, j);
				addAlternate(0);
			}
			newAlternate.CopyFrom(newTile);
			...
			addTile(320);
			newTile.CopyFrom(Style2xX);
			newTile.HookPostPlaceMyPlayer = new PlacementHook(TETrainingDummy.Hook_AfterPlacement, -1, 0, false);
			newTile.Direction = TileObjectDirection.PlaceLeft;
			newTile.StyleMultiplier = 2;
			newTile.StyleWrapLimit = 2;
			newTile.DrawYOffset = 2;
			...
			addTile(14);
			newTile.CopyFrom(Style3x2);
			newTile.HookCheck = new PlacementHook(Chest.FindEmptyChest, -1, 0, true);
			newTile.HookPostPlaceMyPlayer = new PlacementHook(Chest.AfterPlacement_Hook, -1, 0, false);
			newTile.AnchorInvalidTiles = new int[1]
			{
				127
			};
			newTile.LavaDeath = false;
			...
			newTile.CopyFrom(Style2x2);
			newTile.StyleHorizontal = true;
			newTile.HookPostPlaceMyPlayer = new PlacementHook(TEItemFrame.Hook_AfterPlacement, -1, 0, true);
			newAlternate.CopyFrom(newTile);
			newAlternate.Origin = Point16.Zero;
			newAlternate.AnchorTop = new AnchorData(AnchorType.SolidTile | AnchorType.SolidSide, 2, 0);
			newAlternate.AnchorBottom = AnchorData.Empty;
			...
			newTile.CoordinateWidth = 16;
			newTile.CoordinatePadding = 2;
			int[] array6 = newTile.AnchorValidWalls = new int[1];
			newTile.AnchorValidTiles = new int[2]
			{
				2,
				109
			...
			newTile.CoordinateWidth = 16;
			newTile.CoordinatePadding = 2;
			newTile.AnchorValidTiles = new int[1]
			{
				2
			};
			newTile.StyleHorizontal = true;
		...
		{
			NetMessage.SendData(17, -1, -1, "", 1, tileX, tileY, type, style);
